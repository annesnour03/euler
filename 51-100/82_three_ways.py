...
def solve(grid):
    ...
    all_time_shortest = float('inf')
    for start in g.get_left_column_indexes():
        result = dijkstra(g, start)
        g.visited = []

        right_indexes = g.get_right_column_indexes()
        pairs = sorted([(idx, result[idx])
                        for idx in right_indexes], key=lambda x: x[1])
        shortest_pair = pairs[0]
        if shortest_pair[1] < all_time_shortest:
            all_time_shortest = shortest_pair[1]
